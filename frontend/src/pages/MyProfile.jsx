...
const MyProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/user/getUserProfile', {
          headers: {
  ...
  const handleProfileUpdate = async() => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/user/updateUserProfile', {phone,addressLine1,addressLine2,dob,gender,image}, {headers: {token: localStorage.getItem('token')}});
      if (response.data.success) {
